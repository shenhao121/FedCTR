...
def split_exogenous_by_indices(exogenous_data: pd.DataFrame, area_indices: Dict[str, np.ndarray], identifier: str = "District"):
...
def get_exogenous_data_by_area(exogenous_data_train: pd.DataFrame,
                               exogenous_data_val: Optional[pd.DataFrame] = None,
                               identifier: Optional[str] = "District",
                               clients_per_area: int = 5) -> Union[
    Tuple[Dict[str, pd.DataFrame], Dict[str, pd.DataFrame]],
    Dict[str, pd.DataFrame]]:
    """Generates the exogenous data per area and splits each area into multiple clients."""
    if exogenous_data_val is not None:
        assert list(exogenous_data_val[identifier].unique()) == list(exogenous_data_train[identifier].unique())
    ...
    for area in exogenous_data_train[identifier].unique():
        ...
        # 按时间序列划分为多个子客户端
        num_samples_train = len(exogenous_data_train_area) // clients_per_area
        num_samples_val = len(exogenous_data_val_area) // clients_per_area if exogenous_data_val_area is not None else 0
